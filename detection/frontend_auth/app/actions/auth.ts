...
export async function signup(prevState: FormState, formData: FormData): Promise<FormState> {
  ...
  try {
    ...
    const response = await fetch(`https://unabused-profamily-jonna.ngrok-free.dev/auth/signup`, {
      method: 'POST',
...
export async function login(prevState: FormState, formData: FormData): Promise<FormState> {
  ...
  try {
    // Call the local backend server directly
    const response = await fetch(`https://unabused-profamily-jonna.ngrok-free.dev/auth/login`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
